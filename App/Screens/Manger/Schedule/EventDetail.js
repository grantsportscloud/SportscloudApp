...
export default function EventDetailScreen(props) {
  ...
  const [dobValue, setDob] = useState(props.route.params.data.day);
  ...
  const [startTime, setStartTime] = useState(props.route.params.data.time1);
  const [endTime, setEndTime] = useState(props.route.params.data.time2);
  ...
  const addNewGame = async (values) => {
    ...
    if (userData) {
      const obj = {
        name: sportsName,
        short_label: '',
        _id: props.route.params.data._id,
        ...
        <View style={{marginTop: 30}}>
          <Formik
            initialValues={{
              uniform: '',
              note: '',
              locationDetail: props.route.params.data.place,
              Assignments: props.route.params.data.assignment,
            }}
